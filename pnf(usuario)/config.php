...
if(isset($_POST['btn'])){
    ...
    if(strlen($pnombre) > 1 && strlen($snombre) > 1 && strlen($papellido) > 1 && strlen($sapellido) > 1 && strlen($fecha) > 1 && strlen($ci) > 1 && strlen($contraseña) > 1 && isset($sexo)){
        ...
        $consulta = $conexion->query("UPDATE usuarioinformacion SET primernombre = '$pnombre', segundonombre = '$snombre',primerapellido = '$papellido', segundoapellido = '$sapellido',fecha = '$fecha', sexo = '$sexo' where usuarioinfoid like '$id'");
